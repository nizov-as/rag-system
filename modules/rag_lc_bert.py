...
class Doc_Base():
    ...
    def __init__(self, config, name='doc_base', path="./data"):
        ...
        self.tokenizer = AutoTokenizer.from_pretrained(self.config.late_chunk_model_name)
...
class SearchBase():
    ...
    def __init__(self, config):
        ...
        _old_layer_norm_init = nn.LayerNorm.__init__

        def patched_layer_norm_init(self, normalized_shape, eps=1e-5, elementwise_affine=True, *args, **kwargs):
            kwargs.pop("bias", None)
            _old_layer_norm_init(self, normalized_shape, eps=eps, elementwise_affine=elementwise_affine, *args, **kwargs)

        nn.LayerNorm.__init__ = patched_layer_norm_init

        config = AutoConfig.from_pretrained("deepvk/RuModernBERT-base")
    ...
    def search_doc_late(self, qst, doc_base):
        ...
        Возвращает:
            Список из 30 найденных чанков (объектов Doc) с наивысшей схожестью.
        """
        self.doc_base = doc_base
        self.config.reload()
        ...
        def mean_pooling(hidden_states, expanded_mask, seg_start, seg_end):   
            """
        ...
        self.late_chunk_embedder = self.late_chunk_embedder.eval()
        ...
        for src, chunks in source_to_chunks.items():
            ...
            with torch.no_grad():
                outputs = self.late_chunk_embedder(**inputs, output_hidden_states=True)
                # Выбираем последний скрытый слой, shape: [batch_size, sequence_length, hidden_dim]
            ...
            for doc in chunks:
                ...
                pooled = mean_pooling(hidden_states, expanded_mask, seg_start, seg_end)
                candidate_docs.append(doc)
                candidate_embeddings.append(pooled)
            
            del inputs, outputs, hidden_states, attention_mask, expanded_mask
        ...
        qst_inputs = self.late_chunk_tokenizer(qst, return_tensors="pt", padding=True, truncation=True)
        qst_inputs = {key: value.to(device) for key, value in qst_inputs.items()}

        with torch.no_grad():
            qst_outputs = self.late_chunk_embedder(**qst_inputs, output_hidden_states=True)
            # Выбираем последний скрытый слой, shape: [batch_size, sequence_length, hidden_dim]
            qst_hidden_states = qst_outputs.hidden_states[-1].squeeze(0)  # [seq_length, hidden_dim]

        qst_attention_mask = qst_inputs["attention_mask"].squeeze(0) 
        qst_expanded_mask = qst_attention_mask.unsqueeze(-1).expand(qst_hidden_states.size()).float()
        query_embedding_pooled = mean_pooling(qst_hidden_states, qst_expanded_mask, 0, qst_hidden_states.size(0))
        
        # Векторизованный расчёт косинусного сходства на GPU
        # Нормализуем эмбеддинги
        query_embedding_norm = query_embedding_pooled / (query_embedding_pooled.norm() + 1e-9)
        candidate_norms = candidate_embeddings / (candidate_embeddings.norm(dim=1, keepdim=True) + 1e-9)
        # Косинусное сходство: скалярное произведение нормализованных векторов
        similarities = torch.matmul(candidate_norms, query_embedding_norm.unsqueeze(-1)).squeeze(-1)  # [num_candidates]
        
        # Выбираем топ-30 чанков по схожести
        topk = min(30, similarities.size(0))
        top_sim, top_indices = torch.topk(similarities, k=topk, largest=True)
        top_chunks = [candidate_docs[i] for i in top_indices.tolist()]
